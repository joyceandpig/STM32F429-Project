...
u8 updata_fontx(u16 x,u16 y,u8 size,u8 *fxpath,u8 fx)
{
	...
	u32 offx=0;
	u8 rval=0;	     
	fftemp=(FIL*)mymalloc(SRAMIN,sizeof(FIL));	//·ÖÅäÄÚ´æ	SRAMIN
	if(fftemp==NULL)rval=1;
	tempbuf=mymalloc(SRAMIN,4096);				//·ÖÅä4096¸ö×Ö½Ú¿Õ¼SRAMINä
	...
	myfree(SRAMIN,fftemp);	//ÊÍ·ÅÄÚ´æSRAMIN
	myfree(SRAMIN,tempbuf);	//ÊÍ·ÅÄÚ´æSRAMIN
	return res;
} 
...
{	
	...
	res=0XFF;		
	ftinfo.fontok=0XFF;
	pname=mymalloc(SRAMIN,100);	//ÉêÇë100×Ö½ÚÄÚ´æ  SRAMIN
	buf=mymalloc(SRAMIN,4096);	//ÉêÇë4K×Ö½ÚÄÚ´æ SRAMIN 
	fftemp=(FIL*)mymalloc(SRAMIN,sizeof(FIL));	//·ÖÅäÄÚ´  SRAMIN
	if(buf==NULL||pname==NULL||fftemp==NULL)
	{
		myfree(SRAMIN,fftemp);//SRAMIN
		myfree(SRAMIN,pname);//SRAMIN
	...
	myfree(SRAMIN,fftemp);	//ÊÍ·ÅÄÚ´SRAMINæ
	...
	myfree(SRAMIN,pname);//ÊÍ·ÅÄÚ´æ SRAMIN
	myfree(SRAMIN,buf);//SRAMIN
	return rval;//ÎŞ´íÎó.			 
} 
