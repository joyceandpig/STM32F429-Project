...
//应该保证堆栈不小于100个字节.
////////////////////////////////////////////用户配置参数////////////////////////////////////////////////////	  
#define MAX_FNAME_LEN 		64	//函数名最大长度，应该设置为不小于最长函数名的长度。											   
...
extern const u16 USMART_RECV_LEN_MAX;				//在usmart_config.c里面定义
extern u8 USMART_RX_BUF[];     //接收缓冲,最大USART_REC_LEN个字节.//在usmart_config.c里面定义
...
#define USMART_RECV_FLAG_ENTER	0x0001
#define USMART_RECV_FLAG_ORE	0x0002
#define USMART_RECV_FLAG_TAB	0x0004
#define USMART_RECV_FLAG_OVER	0x8000

extern void USMART_Send_Data(u8 data);//必须用户实现
	
static void USMART_RecvInit(u8 * Ptr,u32 MaxLen);//USMART_RecvInit
static u8* USMART_RecvBuffAdd(u16 *RecvLen);
static void USMART_ModifyRecvBuff(u8 *Ptr);//修改接收缓存
	
u8 USMART_StatusCheck(u16 flag);//状态检查
void USMART_StatusClear(u16 flag);//标志清除

void USMART_Recv_Process(u8 data);//接收处理函数，移植时把该函数放到串口接收终端在即可
