...
u8 FTL_Init(void)
{
	...
	if(nand_dev.lut)myfree(SRAMEX,nand_dev.lut);//SRAMIN
	nand_dev.lut=mymalloc(SRAMEX,(nand_dev.block_totalnum)*2); 	//给LUT表申请内存SRAMIN
...
u32 FTL_SearchBadBlock(void)
{
	...
	u32 i,j; 
	u32 goodblock=0;
	blktbl=mymalloc(SRAMEX,nand_dev.block_totalnum);//申请block坏块表内存,对应项:0,好块;1,坏块;SRAMIN
	NAND_EraseChip(); 						//全片擦除
