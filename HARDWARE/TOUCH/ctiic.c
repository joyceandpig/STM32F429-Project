...
{					     
    ...
    CT_IIC_GPIO_SCL_CLK_ENABLE();			//开启GPIOH时钟
		CT_IIC_GPIO_SDA_CLK_ENABLE();			//开启GPIOI时钟
    
    GPIO_Initure.Pin=CT_IIC_GPIO_SCL_PIN;            //PH6
    GPIO_Initure.Mode=GPIO_MODE_OUTPUT_PP;  //推挽输出
    GPIO_Initure.Pull=GPIO_PULLUP;          //上拉
    GPIO_Initure.Speed=GPIO_SPEED_HIGH;     //高速
    HAL_GPIO_Init(CT_IIC_GPIO_SCL_PORT,&GPIO_Initure);     //初始化
	
    GPIO_Initure.Pin=CT_IIC_GPIO_SDA_PIN;            //PI3
    HAL_GPIO_Init(CT_IIC_GPIO_SDA_PORT,&GPIO_Initure);     //初始化
    CT_IIC_SDA_SET;	  	  
		CT_IIC_SCL_SET;
}
//产生IIC起始信号
void CT_IIC_Start(void)
{
	...
	CT_Delay();
	CT_IIC_SCL_CLEAR;//钳住I2C总线，准备发送或接收数据 
}	  
//产生IIC停止信号
