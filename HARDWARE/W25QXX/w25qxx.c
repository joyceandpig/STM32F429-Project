...
{ 
	...
	HAL_GPIO_Init(W25QXX_CS_GPIO_PORT, &GPIO_InitStruct);//PF6推挽输出 
...
void W25QXX_Test(u8 n)
{
	...
	{
		trytime ++;
		u_printf(INFO,"W25QXX_ReadID is Error!TryTime %d",trytime);
		Sleep(100);
		if(trytime == 10)
			break;
	}	
	if(trytime < 10)
	{
		W25QXX_Write(TxBuf,FLASH_SIZE-300,255);		//从倒数第100个地址处开始,写50长度的数据
		W25QXX_Read(RxBuf,FLASH_SIZE-300,255);		//从倒数第100个地址处开始,读50个字节
		u_printf(INFO,"%s",RxBuf);
		
		if(strcmp((const char *)TxBuf,(const char *)RxBuf) == 0)
			u_printf(INFO,"W25QXX Test Write/Read is ok!");
		else
			u_printf(INFO,"W25QXX Test Write/Read is Error!");
	}
	else
		u_printf(INFO,"Exit W25QXX Test");
}
#endif
