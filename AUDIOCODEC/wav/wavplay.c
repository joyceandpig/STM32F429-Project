...
u8 wav_decode_init(u8* fname,__wavctrl* wavx)
{
	...
	ChunkFACT *fact;
	ChunkDATA *data;
	ftemp=(FIL*)mymalloc(SRAMIN,sizeof(FIL));//SRAMIN
	buf=mymalloc(SRAMIN,512);//SRAMIN
	if(ftemp&&buf)	//内存申请成功
	{
	...
	f_close(ftemp);
	myfree(SRAMIN,ftemp);//释放内存SRAMIN
...
u8 wav_play_song(u8* fname)
{
	...
	audiodev.saibuf1=mymalloc(SRAMIN,WAV_SAI_TX_DMA_BUFSIZE);//SRAMIN
	audiodev.saibuf2=mymalloc(SRAMIN,WAV_SAI_TX_DMA_BUFSIZE);//SRAMIN
	audiodev.tbuf=mymalloc(SRAMIN,WAV_SAI_TX_DMA_BUFSIZE);//SRAMIN
	...
	myfree(SRAMIN,audiodev.tbuf);	//释放内存  SRAMIN
	myfree(SRAMIN,audiodev.saibuf1);//释放内存SRAMIN
	myfree(SRAMIN,audiodev.saibuf2);//释放内存SRAMIN 
	myfree(SRAMIN,audiodev.file);	//释放内存  SRAMIN
	return res;
} 
